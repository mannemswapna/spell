...
class spell:
    def __init__(self,root):
        ...
        self.back_btn = Button(root, text='Back', font=('Roboto', 15), command=self.goBack)
        self.back_btn.place(x=100, y=300)
        self.corr_word = None
    ...
    def goBack(self):
        if self.corr_word:
            self.corr_word.destroy()
        self.title_lbl = Label(root, text='SPELLING CHECKER', font=('Roboto', 15, 'bold'))
        self.title_lbl.place(x=180, y=0)
        self.name_entry = Entry(root, width=15, font=('Roboto', 15, 'bold'))
        self.name_entry.place(x=300, y=100)
        self.name_lbl = Label(root, text='Enter a reference word : ', font=('Roboto', 15, 'italic'))
        self.name_lbl.place(x=50, y=100)
        self.submit_btn = Button(root, text='Submit', font=('Roboto', 15), command=self.register)
        self.submit_btn.place(x=250, y=300)
root = Tk()
